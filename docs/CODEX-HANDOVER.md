...
- **Prediction v2/v3**: live forest data (NIBIO SR16 for NO, CORINE for SE), empirical phenology (timing AUC ~0.89), flush timing, per-species scoring, soil-moisture index, occurrence kernel — live. `computeCellPrediction` is the shared brain for both live compute and precomputed tiles.
...
- **Real data over pseudo-noise in the prediction.** Forest (NIBIO/CORINE), elevation (Kartverket), soil-moisture (from daily precip + ET), and empirical phenology (learned weekly curves from 315k GBIF finds) replace hand-wavy proxies. **But the honest validated signal is temporal (timing), not spatial** — occurrence density is a *boost-only* soft prior (~0.52 AUC, mostly accessibility bias). This is documented deliberately to avoid over-claiming.
...
- **Prediction spatial ("where") signal** — computed and shown, but honestly near-chance once accessibility bias is removed (~0.52 AUC). Temporal is the real product argument.
...
- **`computeCellPrediction()` (`cell-score.ts`)** pipeline: `baseScore × speciesFit × habitatFit × hostGate × occurrenceBoost`, clamped 0–100. `baseScore = environment + historical + seasonal`; `environment` blends legacy weather score with advanced factors (soil from bonitet, vegetation from volume, terrain from elevation).
...
- **Occurrence boost (`occurrences.ts`)** — Gaussian kernel (1.5 km bandwidth, 5 km cutoff). **Boost-only** (presence raises score, absence never lowers it) because GBIF is presence-only + sampling-biased. Explicitly a weak prior (~0.52 AUC).
...
- **The prediction engine's honesty problem.** The validated signal is **temporal** (phenology/flush, timing AUC ~0.89). The **spatial** "where" signal is near chance (~0.52 AUC) once accessibility bias is removed; SGU drainage doesn't discriminate (0.47). **How to handle:** keep marketing "when" (mushroom-day/forecast/flush), don't over-promise "where." The only real spatial lever is a **bias-corrected SDM** (a big modeling project) — a logistic baseline exists (`fit:sdm-logistic`) but isn't production. Before claiming any spatial improvement, run `backtest:full-pipeline` with a region split and beat the target-group background.
...
**Prediction honesty (do not over-claim).** Validated signal is **temporal**: empirical phenology curves (learned from 315k dated GBIF finds, 70 species, latitude-banded) give timing AUC ~0.89; flush timing turns this into a "when to go" forecast. The **spatial** "where" signal is near chance (~0.52 AUC) once accessibility bias is removed; the occurrence boost is *boost-only* (presence raises score, absence never lowers) and Swedish forest data (CORINE) is coarser than Norwegian (NIBIO SR16). The only real spatial lever left is a bias-corrected SDM (big project). Market "when," not "where."
