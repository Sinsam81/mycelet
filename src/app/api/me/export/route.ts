...
export async function GET(request: NextRequest) {
  ...
  const datasets = {
    ...
    // Bruksdager (migrasjon 064): dag + flate der soppforholdene ble vist. Egne
    // rader via RLS. Nøkkelen er sammensatt, så sorteringen må være hele nøkkelen
    // — sider sortert på bare `dag` kan gjenta eller hoppe over rader på grensen.
    usageDays: {
      shape: 'many',
      query: hentAlle((fra, til) =>
