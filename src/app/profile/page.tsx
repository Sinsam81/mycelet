...
export default async function ProfilePage() {
  ...
  const [{ data: profile }, statsRes, findingsRes, postsRes, subscription, roleRes] = await Promise.all([
    ...
    supabase
      ...
      // Slettede funn (migrasjon 056) skal ikke stå i «siste funn». Telleren
      ...
      <section className="space-y-4">
        ...
        <article className="rounded-2xl bg-white p-4 shadow-card">
          <div className="flex items-center justify-between">
            ...
            {/* Links to /pricing on BOTH web and native — the pricing page now
                sells via Apple IAP in the shell, so 3.1.1 no longer forbids it. */}
