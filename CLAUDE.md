...
- Keep authed tests **read-only** — there's one Supabase project (no staging), so writes hit prod. Write flows (post forum, save finding, real AI upload) stay manual (🖐️ in the checklist). Usage measurement (`<RegistrerBruksdag>` → `bruksdager`) skips `navigator.webdriver` browsers for the same reason — don't remove that guard.
- **Gotcha:** `/soppforhold` server-renders by fetching its own `/api/prediction/regions`, so `SOPPFORHOLD_BASE` must point at localhost in dev. It used to be hardcoded to production, which meant the page showed prod data no matter what you changed locally — a verdict change gave a fresh API response and a stale page on the same machine, and the fix looked like it hadn't worked. Preview deploys still read production on purpose (a build fetching its own not-yet-deployed URL is chicken-and-egg).
...
1. **Primary:** query `prediction_tiles` table via the `get_prediction_tiles_in_bounds` RPC. Tiles are generated nightly by `/api/cron/generate-tiles?country=NO|SE` (schedules in `vercel.json`, 01:15/01:45 UTC).
...
**Three distributions, one 0–100 label.** The same-looking score has different distributions depending on the surface, and each needs its own ladder. A raster *cell* (`cell-score.ts`) is graded by `CONDITION_THRESHOLDS`; a *region* on `/soppforhold` is the 90th percentile over cells and is graded by `REGION_CONDITION_THRESHOLDS` (`src/lib/prediction/region-score.ts`), measured at p90/p75/median of 374 real region-days; the *point day-score* from `mushroom-day.ts` has its own `forecast-scale.ts` bands. Mixing them is a real bug that shipped: the region endpoint reused the per-cell ladder, so a verdict promising "top 10%" fired on 22.5% of region-days, and the colour came from a third ladder entirely — the same number could get the peak verdict in text and an amber dot beside it. Recalibrate with `npm run calibrate:regionterskler` after the season; the current basis is a 17-day August window. The alert threshold (`VARSEL_MIN_SCORE` in `src/lib/alerts/decision.ts`) is bound to the region ladder's `excellent` for the same reason: it used to import the point-scale `FORECAST_GREEN_MIN` (85), which on the region distribution is the top 12 % of days — 7 of 22 regions (Oslo, Stockholm) never reached it in a month, so alerts and X posts were silent in the biggest markets.

**SR16 lookups are header-keyed.** NIBIO's GetFeatureInfo omits a layer's whole sub-document when it is nodata at the pixel (it does not return −9999), so a positional parse shifts every later value — the original parser read standing volume as site index in 0–14 % of forest cells, which cleared the rich-soil bonus and corrupted the soil factor in the nightly tiles. `parseSr16Html` keys on the «Skogressurskart (<LAYER> raster)» headers (normalising NIBIO's `SSRVOLMB` typo). Cells with treslag nodata but bonitet/volume present are forest and are kept as `ukjent`. Background and the patch-level signal study: `docs/mikro-prediksjon-gjennomgang.md`.

**Forest is sampled at up to five points per grid cell.** SR16 and CORINE are point lookups, and every grid — the nightly raster, `/api/prediction/grid` and `/api/prediction/species-spots` — used to look up only the cell centre and drop the whole cell when that point was water, a field, a road or a settlement, leaving holes over forested land (Oslo map, 15 Sep 2026: Nesodden's east coast, east of Lørenskog, around Enebakk). All three now go through `provSkogIRuter` (`src/lib/prediction/skogprover.ts`): every centre first, then for misses the four quadrant centres (±step/4) in fixed order, first hit wins; cells with no forest at any of the five points stay honest holes (lakes, the fjord, city centres). The cell keeps its centre — grid position, score inputs and pin are unchanged; only the forest data comes from the offset point, which tiles record as `metadata.skogprove` (`senter` | `forskjovet`) + `skogprove_lat`/`skogprove_lng` and which `nearestForestTile`, the grid's area report and the pin explanations in both live grids use for the distance. The pin text must get `skogavstandKm(cell, prove)` — null only when the forest was measured at the centre, because `null` is what prints «Skog her» (and `0` would print «0 m unna»); a live-grid quadrant centre is several km from the pin. Measured live with `npm run dekning:rutenett` (no writes): Oslo 53 → 76 of 84 cells, Bergen 16 → 35 of 40, all of Norway 181 → 317 of 399, Sweden 219 → 354 of 420 uncapped. Offsets run in rounds (first quadrant for every miss, then the second…) so a cap spreads over the region instead of being spent on the first cells and on lakes. They are bounded per region by `EKSTRA_SKOGOPPSLAG_PER_RUTE` (no real cap for SR16; 0.65 per cell for CORINE, which is ~10× slower — ~99 of Sweden's 135 recoverable cells) and by a deadline that reserves time for the remaining regions' centres (`skogproveFrist`); cells cut short are logged as `generate_tiles.skogprove_avkortet`, never dropped silently. The live grids stop starting offset lookups 6 s into the forest phase, so a struggling NIBIO is not hit four times harder.
...
**Photo attribution.** Every species photo is a Wikimedia Commons file under CC BY / CC BY-SA — licenses that require naming the photographer and the license wherever the image is shown. The credit lives beside the URL in both tables: `species_photos.{photographer,license,source_url}` and the denormalized `mushroom_species.primary_image_{photographer,license,source_url}` (migration 062, originally numbered 055 — renumbered because two files shared 055). Backfill both from the Commons API with `node --env-file=.env.local scripts/backfill-photo-credits.mjs` — dry-run by default, `--apply` to write, idempotent, and it never overwrites a hand-set credit without `--force`. Render with `<PhotoCredit>` (`src/components/ui/PhotoCredit.tsx`); it stays silent on placeholder values, because the pre-backfill seed wrote `license = 'Wikimedia Commons'` — a *source*, not a license — and rendering that would be a false licensing claim. Kindwise's `similar_images` carry no credit we can pass on, so they must never inherit one.

**Sanketips articles + AEO.** Source of truth is markdown in `content/sanketips/`; `node scripts/build-articles.mjs` renders `public/landing/sanketips/*.html` AND writes `src/lib/sanketips/manifest.generated.ts` + `fulltekst.generated.ts`, which feed `sitemap.xml`, `/llms.txt` and `/llms-full.txt`. Never hand-list articles anywhere. Each article ends with an H2 «Spørsmål og svar» (sv: «Frågor och svar») of H3 questions + one-paragraph answers drawn from the article itself — rendered visibly and emitted as FAQPage JSON-LD. `ORGANISASJON` in `src/lib/seo/organisasjon.ts` is the single Organization schema (the build script mirrors it; `src/lib/sanketips/__tests__/manifest.test.ts` catches drift). Status and owner tasks in `docs/aeo.md`.
...
**Security headers + CSP** (`next.config.js`). Six baseline headers on every response: HSTS, X-Frame-Options: SAMEORIGIN, X-Content-Type-Options: nosniff, Referrer-Policy: strict-origin-when-cross-origin, Permissions-Policy (camera + geolocation for self), X-XSS-Protection: 0. Plus an **enforcing** `Content-Security-Policy` (flipped from report-only in June 2026) with allowlists for Stripe, Supabase, Wikimedia, Kindwise, Kartverket, OSM, GA4. A new third-party host MUST be added to the allowlist before deploy — otherwise the browser blocks it silently (no report-only safety net any more).

**GDPR endpoints.**
- `/api/me/export` (Art. 15): authenticated GET; returns a JSON file (Content-Disposition: attachment) with every row tied to the user's id across `profiles`, `findings`, `forum_posts`, `comments`, `post_likes`, `comment_likes`, `saved_posts`, `reports` filed, `billing_subscriptions`, `moderator_roles`, `verified_foragers`, `spot_feedback`, `account_deletion_warnings`, `ai_identifications`, `identifications`, `alert_subscriptions` (by user_id **and** by e-mail, so account-less signups are included; tokens stripped) and `blocked_users`, plus `user_metadata` (kilde, terms consent) under `account`. Multi-row datasets are paginated (PostgREST caps at 1000 rows silently — a truncated export would otherwise be labelled complete). All but `ai_identifications`/`alert_subscriptions` are read with the session client (RLS is the source of truth); those two are read with the admin client filtered on the caller's id/e-mail — `ai_identifications` has RLS with no policies and account-less alert rows have no user_id, so through the session client they would come back empty. Excludes reports filed by *others* about the user (would expose reporter) and `admin_audit_log` rows mentioning the user (they identify the acting administrator too).
...
- `/personvern` page: complete privacy policy in **both nb and sv** — no `[FYLL INN]` placeholders remain (verified: `src/lib/legal/__tests__/entity.test.ts` asserts the rendered legal text contains none). Real controller + org number, all four Art. 6 bases, nine named processors with DPA links, concrete retention periods, and the right supervisory authority per language (Datatilsynet on nb, IMY on sv). Still needs a **lawyer review** before launch — that is the only outstanding item, not filling anything in.
- `CookieNotice` component (mounted in root layout, web only via `NonNativeOnly`): a **consent dialog for optional GA4 analytics** — Google Analytics is not loaded until the visitor accepts (`src/components/analytics/Analytics.tsx`, key `mycelet:analytics-consent-v1`), and the choice can be changed from `/personvern`. The native app has no analytics and no dialog. Strictly-necessary cookies (session, locale) need no consent and are only described.

**Usage measurement (bruksdager).** `bruksdager` (migration 064) holds one row per user, Oslo date and surface (`hjem` | `kart` | `omrade` | `steder` | `pris` | `tilbud`, plus region slug for area pages) — no coordinates, no timestamps beyond the date. Written by `/api/me/bruksdag` (session client, RLS own-row, `ignoreDuplicates`) from the client component `<RegistrerBruksdag>` rendered inside the home forecast card (only once it has data), on `/map`, on `/soppforhold/[omrade]`, on `/mine-steder` (migration 066) and on `/pricing` (migration 067) — plus `tilbud` written by `<ProvGratisArk>` when the trial sheet is shown (migration 068). `pris` and `tilbud` are funnel surfaces: they count in `perFlate` but never in «kom tilbake»/reuse; a client component on purpose, because the home card and the area pages fetch the forecast anonymously and `/api/prediction` serves logged-out users too. It skips automated browsers (`navigator.webdriver`), which is what keeps the authed Playwright project read-only against the single prod database. The daily report derives «kom tilbake» (a usage day *later* than the registration day — the home card renders automatically on day one) and «gjenbruk» (two distinct ISO weeks in 28 days) in `src/lib/rapport/dagsrapport.ts`. Registrations from the native shell get `user_metadata.kilde = 'app'` and `plattform` (`registreringsKilde` in `src/lib/analytics/kilde.ts`); before that every app signup landed in «direkte / ukjent». Web signups with no source cookie get `kilde = 'web:direkte'` (since 14 Sep 2026; older rows stay `ukjent`), and every signup also records `sprak` (active locale) and `tidssone` (the device's IANA zone — Europe/Oslo vs Europe/Stockholm is the only honest country signal; absent for OAuth accounts, which are stamped server-side). Both are described in `/personvern` and included in the GDPR export (`usageDays`); the App Store privacy label must declare «Product Interaction» accordingly.

**Anonymous pre-account counter (flatetellinger).** `flatetellinger` (migration 070) holds one integer per Oslo date, surface (`soppforhold` = the shell's logged-out first screen, `register` = the registration form) and language — no user id, device id, IP or timestamp, so it is not personal data and is neither exported nor deleted with an account (it is still mentioned in `/personvern`). It is written only through the `security definer` function `tell_flate` (execute granted to `service_role`; RLS on with no policies) from `POST /api/tell`, which answers 204 no matter what and rate-limits per client key. The client component `<TellFlate flate>` (`src/components/bruk/TellFlate.tsx`) is mounted inside `<NativeOnly>` on `/soppforhold` and `/auth/register` only, once per session, and skips `navigator.webdriver`. The daily report shows it as «Før konto i appen» — the denominator for how many installs ever reach the registration form.

**Paying, trials and gift passes (daily report and /admin).** One rule — `klassifiserAbonnement` in `src/lib/rapport/abonnement.ts` — feeds both the daily report and the `/admin` statistics page; each row lands in exactly one of `betalende` / `prove` / `gratisTildelt` / `utlopt` / `inaktiv`. «Betalende» is a store row (Stripe or App Store) with status `active`, a paid tier and a running period. `trialing` is *not* «betalende» and has its own block (løpende, startet 7 d, til første belastning, avbrutt). Rows marked `metadata.source = 'manual_grant'`, rows without a store provider and rows owned by internal `@mycelet.com` accounts are «gratis tildelt», never sales. `/admin` had its own loop until 15 Sep 2026 and showed 8 paying where the truth was 1 (three trials, three grants, a Stripe row that expired in July). Reporting only: Premium access still goes through `hasPaidAccess`, where trials and grants do unlock features. Both webhooks stamp `metadata.prove_start` when a row first becomes trialing and `metadata.forste_belastning` when a trial converts to a paid `active` (`oppdaterProveMerker` in `src/lib/billing/prove-merke.ts`) — needed because both webhooks rebuild `metadata` on every write. A RevenueCat CANCELLATION during a TRIAL keeps `trialing` so a cancelled free week never reads as a customer.

**Rapportpuls** (`rapportpuls`, migration 069; cron `/api/cron/rapportpuls` 04:40 UTC, Norway only). Per region: GBIF fungi records in the window `[day−13, day−7]` vs the mean of the same window in 2023–25 (`src/lib/rapportpuls/puls.ts`). **Not shown to customers since 17 Sep 2026** — the alert e-mail and the X flip post no longer receive it. The original lag reasoning was wrong: the gap is not GBIF ingestion but people entering finds into Artsobservasjoner weeks to months after the trip (by mid-September only 57–74 % of a year's August records exist; ~95 % by New Year). Comparing an unfinished current window with finished prior years therefore reads «under» almost everywhere every autumn (Kristiansand −92 % went out in two alert e-mails on 15–16 Sep), and the small region boxes are dominated by single recorders (Stavanger +555 % on a baseline of 20). A fair comparison counts prior years *as they stood on the same date*, using Artsobservasjoner's sighting id (GBIF `catalogNumber`, dataset `b124e1e0-4755-430f-9eab-894f25a9b59c`), which rises in entry order. Never re-expose a year-over-year number outside the owner's report without that same-date baseline.
...
Pre-generated `prediction_tiles` store habitat reasons in **both** languages since PR #118 (`reasons` + `reasonsSv` in tile metadata), and the tile path of `/api/prediction` picks by locale — so every path is localized. Tiles older than 2026-08-02 lacked `reasonsSv`, but the raster regenerates nightly, so that gap closed itself.
...
- **Never run `npx cap sync` from a git worktree.** It rewrites the relative paths in `ios/App/CapApp-SPM/Package.swift` to match the worktree's depth (`../../../../../../node_modules` instead of `../../../node_modules`). The file is tracked, so committing that breaks the iOS build for everyone in the main checkout. `Info.plist` is safe — sync leaves it byte-identical (only `cap add` rewrites it, via `editProjectSettingsIOS`) — but if you do run sync in a worktree, `git checkout -- ios/App/CapApp-SPM/Package.swift` afterwards.
