...
**Trials in the daily report.** `billing_subscriptions.status = 'trialing'` is *not* «betalende»: the report counts only `active` rows with a running period, and shows trials in their own block (løpende, startet 7 d, til første belastning, avbrutt). Both webhooks stamp `metadata.prove_start` when a row first becomes trialing and `metadata.forste_belastning` when a trial converts to a paid `active` (`oppdaterProveMerker` in `src/lib/billing/prove-merke.ts`) — needed because both webhooks rebuild `metadata` on every write. A RevenueCat CANCELLATION during a TRIAL keeps `trialing` so a cancelled free week never reads as a customer.

**Rapportpuls** (`rapportpuls`, migration 069; cron `/api/cron/rapportpuls` 04:40 UTC, Norway only). Per region: GBIF/Artsobservasjoner fungi records (all species) in the window `[day−13, day−7]` vs the mean of the same window in the three previous years, plus a trend against the row stored seven days earlier (`src/lib/rapportpuls/puls.ts`). **The window lags a week on purpose:** GBIF fills Norwegian records in over ~7 days (measured 14 Sep 2026: a window ending today holds ~40 % of what a week-old window holds — numbers in the migration header), so a fresh window compared with complete prior-year windows would always read «under». The trend compares two rows of the *same age*, never a fresh count against an old one. `tynt` below a baseline of 10; `over` at +30 %, `under` only at −40 % because late uploads over months still bias the prior-year comparison slightly low. It is an *explanation* line in the alert e-mail, the X flip post and the daily report — never an alert trigger, because the fasit counts the same records and would then measure itself. Sweden is excluded until Artportalen's GBIF feed stops lagging by weeks. Spatially these records mostly encode where people walk (`docs/mikro-prediksjon-gjennomgang.md`), which is why they are used as a time signal only.
