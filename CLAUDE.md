...
**Saved places** (`saved_places`, migration 055 — **not yet applied in prod**). A marked place is NOT a finding: coordinate + name + note, no species, no observation date, no weather snapshot, and **no visibility model at all** — owner-only RLS, no public view, no display-location trigger. Never let them into `public_findings`, the prediction model, or any aggregate; they must not count in `get_user_stats`. Written by `/api/me/steder` (import/rename/delete/undo-batch), read by `/mine-steder` and the map's places layer. GPX import parses the file **in the browser** (`src/lib/gpx/les-gpx.ts`, `DOMParser`) and the route only ever accepts already-validated JSON — never add server-side XML parsing. Shared validation + caps live in `src/lib/steder/veipunkt.ts` (500 waypoints/import, 1000 places/user — the per-user cap is also a DB trigger, since PostgREST can be called directly). Rationale and the rejected alternatives: `docs/gpx-import-design.md`.
