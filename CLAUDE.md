...
**Three distributions, one 0–100 label.** The same-looking score has different distributions depending on the surface, and each needs its own ladder. A raster *cell* (`cell-score.ts`) is graded by `CONDITION_THRESHOLDS`; a *region* on `/soppforhold` is the 90th percentile over cells and is graded by `REGION_CONDITION_THRESHOLDS` (`src/lib/prediction/region-score.ts`), measured at p90/p75/median of 374 real region-days; the *point day-score* from `mushroom-day.ts` has its own `forecast-scale.ts` bands. Mixing them is a real bug that shipped: the region endpoint reused the per-cell ladder, so a verdict promising "top 10%" fired on 22.5% of region-days, and the colour came from a third ladder entirely — the same number could get the peak verdict in text and an amber dot beside it. Recalibrate with `npm run calibrate:regionterskler` after the season; the current basis is a 17-day August window.
